...
function toggleTurn() {
  ...
  if (turn === "o") {
    ...
    badgeTurn.innerHTML = xIcon + "<h3>TURNO</h3>";
  } else {
    turn = "o";
    newFavicon.setAttribute("href", "icon-o.ico");
    badgeTurn.innerHTML = oIcon + "<h3>TURNO</h3>";
  }
...
function checkWin() {
  ...
  for (let i = 0; i < GameMirror.length; i++) {
    if (
      GameMirror[i][0] === GameMirror[i][1] &&
      GameMirror[i][1] === GameMirror[i][2] &&
      GameMirror[i][0] != null
    ) {
      return turn;
    }
  ...
  // Verifica colunas
  for (let i = 0; i < GameMirror[0].length; i++) {
    if (
      GameMirror[0][i] === GameMirror[1][i] &&
      GameMirror[1][i] === GameMirror[2][i] &&
      GameMirror[0][i] != null
    ) {
      return turn;
    }
  ...
  // Verifica diagonais
  if (
    GameMirror[0][0] === GameMirror[1][1] &&
    GameMirror[1][1] === GameMirror[2][2] &&
    GameMirror[0][0] != null
  ) {
    return turn;
  }
  if (
    GameMirror[0][2] === GameMirror[1][1] &&
    GameMirror[1][1] === GameMirror[2][0] &&
    GameMirror[0][2] != null
  ) {
    return turn;
  }
...
function setPoints(winner) {
  switch (winner) {
    ...
    case "draw":
      ...
      break;
    default:
      console.log("Erro ao setar pontos");
      break;
  }
}

function actionModalWinner(winner) {
  ...
  if (modal.open == false) {
    ...
    section.style.animation = "";
       ...
       switch (winner){
      case "x":
          modal.querySelector("svg").innerHTML = xIcon;
          modal.querySelector("h2").style.color = "#00c2c0";
          modal.querySelector("h2").textContent = `VITÓRIA`;
        break;
      case "o":
          modal.querySelectorAll("svg").remove();
          modal.querySelector("h2").style.color = "#f4b348";
          modal.querySelector("h2").textContent = `VITÓRIA`;
        break;
      case "draw":
          modal.querySelector("svg").innerHTML = "";
          modal.querySelector("h2").style.color = "#a7bec8";
          modal.querySelector("h2").textContent = `EMPATE`;
        break;
    }
